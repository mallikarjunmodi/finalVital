...
import BloodGlucoseStart from "../components/popup/AllTestSeperate/BloodGlucoseStart";
...
const BloodGlucosepopup = (props) => {
  ...
  const [popUpSequence, setPopupSequence] = useState("BG_START");
        ...
        else if (popUpSequence === "BG_INST") return (<BloodGlucoseInstruction setinitateTestPopUp={"wear device and press \"start\"."} onExitClick={props.onExitClick} onContinueClick={()=>{setPopupSequence("BG_END");SensorRead((data)=>{setGlData(data); if(data.state==="end"){setPopupSequence("")} console.log("glData",glData)});}}/>);
 ...
 function  SensorRead(callback){
  ...
  socket.emit("send_message_gl",{message:"Start"});
  
   socket.on("gl_data",(data)=>{
